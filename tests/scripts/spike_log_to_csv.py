...
MEM_RE = re.compile(r"\bmem\s+0x(?P<addr>[a-f0-9]+)(?:\s+0x(?P<data>[a-f0-9]+))?")

LOGGER = logging.getLogger()


def split_operands(entry):
    """Split operand string into rd, rs1, rs2_imm based on instruction type."""
    entry.rd = ""
    entry.rs1 = ""
    entry.rs2_imm = ""

    if not entry.operand:
        return

    parts = [p.strip() for p in entry.operand.split(",")]

    instr = entry.instr

    # rd, imm
    if instr in ["li", "lui", "auipc"]:
        if len(parts) > 0:
            entry.rd = parts[0]
        if len(parts) > 1:
            entry.rs2_imm = parts[1]
        return
    ...
    if instr in [
        ...
        if len(parts) > 1:
            entry.rs1 = parts[1]
        if len(parts) > 2:
            entry.rs2_imm = parts[2]
        return

    # rs2/source, rs1/base, imm for stores
    # Your column name is rd, but for stores this is NOT rd.
    # It is the source data register.
    if instr in ["sb", "sh", "sw", "sd", "fsw", "fsd"]:
        if len(parts) > 0:
            entry.rd = parts[0]  # actually store source register
        if len(parts) > 1:
            entry.rs1 = parts[1]  # base register
        if len(parts) > 2:
            entry.rs2_imm = parts[2]  # offset
        return
    ...
    # Again, rd is not really rd here.
    if instr in ["beq", "bne", "blt", "bge", "bltu", "bgeu"]:
        if len(parts) > 0:
            entry.rd = parts[0]  # actually branch rs1
        if len(parts) > 1:
            entry.rs1 = parts[1]  # actually branch rs2
        if len(parts) > 2:
            entry.rs2_imm = parts[2]  # branch offset/target
        return
...
def process_instr(trace):
    ...
    if m:
        trace.operand = "{},{},{}".format(m.group("rd"), m.group("rs1"), m.group("imm"))
    ...
    with open(path, "r") as handle:
        for line in handle:
            ...
            if commit_match:
                ...
                instr.gpr.append("0x" + groups["val"])
            ...
            if mem_match:
                groups = mem_match.groupdict()
                addr = groups["addr"]
                data = groups["data"]

                if data:
                    instr.mem.append("0x{}:0x{}".format(addr, data))
                else:
                    instr.mem.append("0x{}".format(addr))
    ...
    with open(csv, "w") as csv_fd:
        ...
        for entry, illegal in read_spike_trace(spike_log, full_trace):
            ...
            if illegal and full_trace:
                logging.debug(
                    "Illegal instruction: {}, opcode:{}".format(
                        entry.instr_str, entry.binary
                    )
                )
