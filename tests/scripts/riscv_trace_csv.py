...
class RiscvInstructionTraceCsv(object):
    ...
    def read_trace(self, trace):
        ...
        for row in csv_reader:
            ...
            new_trace.gpr = row["gpr"].split(";") if row["gpr"] else []
            new_trace.csr = row["csr"].split(";") if row["csr"] else []
            new_trace.mem = row["mem"].split(";") if row.get("mem") else []
            new_trace.pc = row["pc"]
            new_trace.operand = row["operand"]
            new_trace.rd = row.get("rd", "")
            new_trace.rs1 = row.get("rs1", "")
            new_trace.rs2_imm = row.get("rs2_imm", "")
        ...
        self.csv_writer.writerow(
            {
                ...
                "rd": abi_to_gpr(entry.rd),
                "rs1": abi_to_gpr(entry.rs1),
                "rs2_imm": abi_to_gpr(entry.rs2_imm),
                "pc": entry.pc,
                "binary": entry.binary,
                "instr": entry.instr,
                "mode": entry.mode,
