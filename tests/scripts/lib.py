...
def run_cmd(
    cmd, timeout_s=3600, exit_on_error=1, check_return_code=True, debug_cmd=None
):
...
def run_parallel_cmd(
    cmd_list, timeout_s=999, exit_on_error=0, check_return_code=True, debug_cmd=None
):
...
def process_regression_list(testlist, test, iterations, matched_list, riscv_dv_root):
    ...
    for entry in yaml_data:
        if "import" in entry:
            ...
            process_regression_list(
                sub_list, test, iterations, matched_list, riscv_dv_root
            )
        else:
            if (entry["test"] in mult_test) or (test == "all"):
                ...
                if entry["iterations"] > 0:
                    logging.info(
                        "Found matched tests: {}, iterations:{}".format(
...
def abi_to_gpr(reg):
    """Convert ABI register name to architectural register."""
    return ABI_TO_GPR.get(reg, reg)
